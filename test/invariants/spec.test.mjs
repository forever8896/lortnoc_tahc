...
describe('§9 — World ID only as an OPTIONAL reader check, never for writing or metering', () => {
  ...
  const ALLOWED = /^(gate\/|shared\/checks\/human\.mjs|extension-everywhere\/(src\/reveal\/|src\/world\/|src\/background\/|src\/shared\/messages\.ts|package(-lock)?\.json|manifest\.config\.ts|vite\.config\.ts))/
